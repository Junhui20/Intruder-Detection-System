...
const acts = {
  ...
  'enrol-pet'() { const classes = JSON.parse($('#animal-classes')?.textContent || '{"16":"dog","15":"cat"}'); const options = Object.entries(classes).map(([id, kind]) => `<option value="${id}"${kind === 'dog' ? ' selected' : ''}>${kind}</option>`).join(''); modal('Enrol a pet', `<form id="enrol-form" method="post" action="/enrol/animal" enctype="multipart/form-data"><div class="form">${field('e-name', 'Name', '<input id="e-name" name="name" required placeholder="Jacky">')}${field('e-kind', 'Kind', `<select id="e-kind" name="class_id">${options}</select>`)}${field('e-photos', 'Three or more photos', '<input id="e-photos" name="photos" type="file" accept="image/*" multiple required>')}</div><div class="help" style="font-size:12px;color:var(--ink-3)">Photos from this camera work best — same angle, same light.</div></form>`, '<button class="btn" data-act="close">Cancel</button><button class="btn primary" data-act="submit-form">Enrol</button>'); },
